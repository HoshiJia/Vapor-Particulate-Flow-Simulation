...
{	
	...
	for (int i = 0; i < num; ++i) {
		...
		dec_x = - (920 - 48.7) / 48.7 * 9.8 / 2; // - 3 / 8 / vpp->radius[i] * 0.44 * 920/48.7 * pow(vpp->loc_y[i],2);
		dec_y = (920 - 48.7) / 48.7 * 9.8 * sqrt(3) / 2; //- 3 / 8 / vpp->radius[i] * 0.44 * 920/48.7 * pow(vpp->loc_y[i],2);
...
int tubeCollisionRe(Particle * vpp, Configure * tbp, int p, int b, int flag, float ratio)
{
	...
	if (b > 1) {
		...
		if (flag == 0 && delta_dist < (tbp->radius+0.008)) {
			vpp->region[p] = b;
			flag = 1;
